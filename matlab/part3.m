...
F = [A-A*D C; D B-B*C];
sys_sir_base = ss(F, zeros(2*n,1),eye(2*n) ,zeros(2*n,1),1);
Z = lsim(sys_sir_base,zeros(d,1),linspace(593,740,d),z0); %simulate for d days of spread
...
plot(Z(1:d,1:5));
legend('Normal', 'Vulnerable','Normal Infected','Vulnerable Infected','Dead');
title('Omicron Wave: Pop 1 without policy');
xlabel('Time')
ylabel('Fraction of Population');
ylim auto; hold off;
...
plot(Z(1:d,7:11));
legend('Normal', 'Vulnerable','Normal Infected','Vulnerable Infected','Dead');
title('Omicron Wave: Pop 2 without policy');
xlabel('Time')
ylabel('Fraction of Population');
ylim auto; hold off;
...
% simulate last 27 days
sys_sir_base = ss(F, zeros(2*n,1),eye(2*n) ,zeros(2*n,1),1);
Z = lsim(sys_sir_base,zeros(27,1),linspace(713,739,27),z0); %simulate for d days of spread

figure;
hold on; %toggle hold, plotting multiple curves on the same graph
plot(Z(:,1:5));
legend('Normal', 'Vulnerable','Normal Infected','Vulnerable Infected','Dead');
title('Omicron Wave: Pop 1 woth gradually implemented policy');
xlabel('Time')
ylabel('Fraction of Population');
ylim auto; hold off;
...
plot(Z(:,7:11));
legend('Normal', 'Vulnerable','Normal Infected','Vulnerable Infected','Dead');
title('Omicron Wave: Pop 2 with gradually implemented policy');
xlabel('Time')
ylabel('Fraction of Population');
ylim auto; hold off;
